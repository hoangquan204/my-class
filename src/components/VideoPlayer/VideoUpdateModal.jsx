...
import { createVideo, updateVideo } from './videoSlice';
...
const getYouTubeVideoId = (url) => {
    try {
      ...
      return `https://www.youtube.com/watch?v=${pathParts[pathParts.length - 1]}`;
    } catch (e) {
      return null;
    }
  }

export default function VideoUpdateModal({ classRoomId, videoPlayer }) {
  const [open, setOpen] = React.useState(false);
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);

  const [title, setTitle] = React.useState(videoPlayer?.title || '');
  const [description, setDescription] = React.useState(videoPlayer?.description || '');
  const [url, setUrl] = React.useState(getYouTubeVideoId(videoPlayer?.url) || '');
  ...
  const handleCreateVideo = () => {
    ...
    dispatch(updateVideo({id: videoPlayer.id, title, description, url }));
    dispatch(
